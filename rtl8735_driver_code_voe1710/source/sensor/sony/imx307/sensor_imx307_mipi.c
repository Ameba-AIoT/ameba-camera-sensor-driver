...
//#define SENSOR_TEMPERATURE_CTRL
...
static struct imx307_status g_status[SUPPORTED_ISP_NUM] = {[0].hdr = LINEAR_MODE};

static struct rts_isp_i2c_reg g_imx307_i2c_init_regs_linear[] = {
	{0x3002, 0x00}, {0x3005, 0x01}, {0x3007, 0x00}, {0x3009, 0x02},
...
static struct rts_isp_i2c_reg g_imx307_i2c_init_regs_hdr[] = {
	...
	{0x3021, 0x00}, {0x3024, 0xC9}, {0x3025, 0x07}, {0x3030, 0x0B},
...
#if 0
static struct rts_isp_i2c_reg g_imx307_i2c_ae_regs[] = {
	//4x-Original
	/*{0x3019, 0x05}, {0x3018, 0x53}, {0x3021, 0x00}, {0x3020, 0x03},
	{0x3025, 0x08}, {0x3024, 0xc9}, {0x3419, 0x09}, {0x3418, 0x24},
	{0x3031, 0x00}, {0x3030, 0x7B}, {0x3014, 0x01},*/

	//32x
	/*{0x3019, 0x05}, {0x3018, 0x53}, {0x3021, 0x00}, {0x3020, 0x5c},
	{0x3025, 0x06}, {0x3024, 0xE5}, {0x3419, 0x09}, {0x3418, 0x24},
	{0x3031, 0x00}, {0x3030, 0x7B}, {0x3014, 0x01},*/

	//8x
	{0x3019, 0x05}, {0x3018, 0x53}, {0x3021, 0x00}, {0x3020, 0x02},
	{0x3025, 0x06}, {0x3024, 0xE5}, {0x3419, 0x09}, {0x3418, 0x24},
	{0x3031, 0x00}, {0x3030, 0x7B}, {0x3014, 0x01},
};
#endif
static int imx307_get_info(uint32_t isp_id, struct rts_isp_sensor_info *info)
{
	...
	info->modes.mode[i].fps = 30.0f; //@MLCK=37.125MHz
	i++;
				   ...
				   struct rts_isp_sensor_init_info *info)
{
	...
	if (mode->hdr == HDR_MODE) {
		...
		info->interface.vhdr_start_line_tnr = 0;
		...
		struct rts_isp_sync_regs *regs)
{
	...
	debug("chan1 exposure = %f\r\n", exp_gain->exposure[0]);
	debug("chan2 exposure = %f\r\n", exp_gain->exposure[1]);
	debug("chan1 anagain = %f\r\n", exp_gain->analog_gain[0]);
	debug("chan2 anagain = %f\r\n", exp_gain->analog_gain[1]);
	debug("shs1 shs2 = %d %d\r\n", shs1, shs2);
	debug("rhs1 yout = %d %d\r\n", rhs1, y_out_size);
	debug("vmax      = %d   \r\n", vmax);
...
static int imx307_set_mirror_flip(uint32_t isp_id,
...
/* Sensor Temperature Control */
#ifdef SENSOR_TEMPERATURE_CTRL
static uint32_t imx307_get_temperature_indicator(uint32_t isp_id)
{
	int ret;
	struct rts_isp_i2c_reg reg;
...
static float imx307_get_again_bound(uint32_t isp_id, uint32_t temperature)
{
	float bound;
	int index;
	uint32_t max_bound = 0;
	uint32_t bounds[] = {0x20, 0x40, 0x60, 0xa0, 0xe0, 0x120};					//temperature value
	uint32_t value[] = {16*16, 12*16, 8*16, 6*16, 4*16, 2*16};					//Gain bound (Unit:1x = 16)

	for (index=0; index < (sizeof(bounds)/sizeof(bounds[0])); index++) {
		if (temperature < bounds[index]) {
			if (value[index] > (ANALOG_GAIN_MAX_RATIO*16))
	...
	if (index == 0) {
		max_bound = value[0];
	} else if (index == sizeof(bounds)/sizeof(bounds[0])) {
		max_bound = value[index-1];
	} else {
		max_bound = linear_intp(index,
					   value[index - 1], value[index - 1],
					   bounds[index], value[index]);
	}
	bound = (float)((float)max_bound / 16.0f);

	return bound;
}

static int imx307_set_again(uint32_t isp_id,
							struct rts_isp_sensor_again *again_info)
{
	...
	val = get_sensor_gain_reg(again_info->again_val);

	i = 0;
	reg = again_info->regs.reg;

	set_sync_i2c(&reg[i++], 0x3014, val);
	printf("%s again %.2f \n", __func__, again_info->again_val);
	again_info->regs.num = i;
...
static const struct rts_isp_sensor_ops imx307_ops = {
	...
	.set_mirror_flip = imx307_set_mirror_flip,
	.check = imx307_check,
#ifdef SENSOR_TEMPERATURE_CTRL
	.get_temperature = imx307_get_temperature_indicator,			/* Sensor Temperature Control */
	.get_bound = imx307_get_again_bound,							/* Sensor Temperature Control */
	.set_again = imx307_set_again,									/* Sensor Temperature Control */
#endif
};
