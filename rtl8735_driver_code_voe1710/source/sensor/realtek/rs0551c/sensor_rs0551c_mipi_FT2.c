...
#define SENSOR_HEIGHT 1512
#define SENSOR_VTS 1540
#if CONFIG_FPGA // FPGA
#define FPS 3
#define PCLK 16000000
#else
...
static struct rs0551c_mipi_status g_status[SUPPORTED_ISP_NUM] = {[0].hdr = LINEAR_MODE};
...
static const struct rs0551c_mipi_config g_rs0551c_mipi_config = {
	...
	.lanes = MIPI_LANE0 | MIPI_LANE1 | MIPI_LANE2 | MIPI_LANE3,
};
...
static int rs0551c_mipi_get_pclk_vts(struct rs0551c_mipi_status *status)
{
	...
	status->mipi_clk = g_rs0551c_mipi_config.pclk * status->divider * 2 / 1000000;

	status->pclk_fix = g_rs0551c_mipi_config.pclk;

	hts = status->pclk_fix/g_rs0551c_mipi_config.vts /
	      g_rs0551c_mipi_config.fps;

	// Even Number and larger fps
	if (hts & 1)
		hts -= 1;
				      ...
				      struct rts_isp_sensor_init_info *info)
{
	...
	printf("rs0551c mipi get fps %.1f init info\n", mode->fps);
...
static int rs0551c_mipi_fixp_set(struct rs0551c_mipi_status *status)
{
	...
	if (status->hts < (g_rs0551c_mipi_config.width << shift) ||
	    (g_rs0551c_mipi_config.width << shift) > 4095)
		return -RTS_ISP_EINVAL;

	dummy_pixel = status->hts - (g_rs0551c_mipi_config.width << shift);
...
static int rs0551c_mipi_run(struct rs0551c_mipi_status *status)
{
	...
	if (g_rs0551c_mipi_config.type == YUV_SENSOR)
		...
		//write_snr_reg(0x0210, 0x80); //color pattern
		write_snr_reg(0x0210, 0xA0); //random data, all frame are same
	// MIPI TX Enable
