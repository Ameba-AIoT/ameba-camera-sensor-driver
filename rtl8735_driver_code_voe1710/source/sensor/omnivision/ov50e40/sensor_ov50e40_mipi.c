...
#include "ov50e40_init_registers_backup.h"
...
static struct ov50e40_status g_status[SUPPORTED_ISP_NUM] = {[0].hdr = LINEAR_MODE};

//TODO: Verify Data
static const struct fps_info g_ov50e40_fps_info_asic[] = {
	//{30.0f, 2347, 160000000},
	{30.0f, 2347, 160000000},
};

static int ov50e40_get_info(uint32_t isp_id, struct rts_isp_sensor_info *info)
{
	int i;
	...
	info->modes.mode[0].fps = g_ov50e40_fps_info_asic[0].fps;
	...
	set_power_item(&up->items[i++], SNR_HCLK, CLK_12M, 3000);
...
static const struct fps_info *ov50e40_get_fps_info(uint16_t fps)
{
	int i;

	for (i = 0; i < ARRAY_SIZE(g_ov50e40_fps_info_asic); i++)
		if (fps == g_ov50e40_fps_info_asic[i].fps)
			break;
	if (i == ARRAY_SIZE(g_ov50e40_fps_info_asic))
		return NULL;
	return &g_ov50e40_fps_info_asic[i];

}

static int ov50e40_get_init_info(uint32_t isp_id,
				const struct rts_isp_sensor_mode *mode,
				struct rts_isp_sensor_init_info *info)
{
	const struct fps_info *fps_info;
	struct ov50e40_status *status;
	...
	fps_info = ov50e40_get_fps_info(mode->fps);
	if (!fps_info)
		return -RTS_ISP_EINVAL;
		
#if FOV_MODE == 0
	set_init_i2c_regs(info->sensor_regs[0], g_ov50e40_i2c_init_regs_asic_4c1, 1000);
#else
	set_init_i2c_regs(info->sensor_regs[0], g_ov50e40_i2c_init_regs_asic_crop, 1000);
#endif
	...
	info->min_vts = status->min_vts = 2272;
...
static int ov50e40_start(uint32_t isp_id)
{
	struct ov50e40_status *status;
...
static uint16_t get_sensor_gain_reg(float fgain, struct ov50e40_status *status)
{
...
static int ov50e40_get_tuned_again(uint32_t isp_id,
...
static int ov50e40_get_tuned_dgain(uint32_t isp_id,
...
static int ov50e40_get_exposure_gain_info(uint32_t isp_id,
...
static int ov50e40_set_mirror_flip(uint32_t isp_id,
...
static int ov50e40_check(uint32_t isp_id)
{
	int ret;
	int id;
...
static const struct rts_isp_sensor_ops ov50e40_ops = {
	.ops_version = SENSOR_OPS_VERSION,
	.name = "ov50e40",

	.get_info = ov50e40_get_info,
	.get_init_info = ov50e40_get_init_info,
	.start = ov50e40_start,
	.get_tuned_again = ov50e40_get_tuned_again,
	.get_tuned_dgain = ov50e40_get_tuned_dgain,
	.get_exposure_gain_info = ov50e40_get_exposure_gain_info,
	.set_mirror_flip = ov50e40_set_mirror_flip,
	.check = ov50e40_check,
