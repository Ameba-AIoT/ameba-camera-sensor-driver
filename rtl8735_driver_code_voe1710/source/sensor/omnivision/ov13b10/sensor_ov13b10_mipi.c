...
#else
	info->modes.mode[i].hdr = LINEAR_MODE;
	info->modes.mode[i].size.w = 2104;     //2104
								...
								struct rts_isp_sensor_mirror_flip *mf_info)
{
	...
	switch(bySnrImgDir)//
	{
			...
			case 0: //normal
			default:
			set_sync_i2c(&reg[i++], 0x3811, 0x67);
			set_sync_i2c(&reg[i++], 0x3813, 0x39);
			set_sync_i2c(&reg[i++], 0x3820, 0x88);
			break;
			case 1: 	// Mirror
			set_sync_i2c(&reg[i++], 0x3811, 0x68);
			set_sync_i2c(&reg[i++], 0x3813, 0x39);
			set_sync_i2c(&reg[i++], 0x3820, 0x80);
			break;
			case 2: 		// VFlip
			set_sync_i2c(&reg[i++], 0x3811, 0x67);
			set_sync_i2c(&reg[i++], 0x3813, 0x38);
