...
static struct jxf38p_status g_status[SUPPORTED_ISP_NUM] = {[0].hdr = LINEAR_MODE};
...
static struct rts_isp_i2c_reg g_jxf38p_i2c_init_regs_asic[] = {
	...
	{0xA5,0x4F},
	{0x20,0x00},
	{0x21,0x05},	//frame width 0x500 = 1280( x 2) = 2560
	{0x22,0x65},
	{0x23,0x04},	//frame height 0x465 = 1125
	{0x24,0xC0},	//HWin=h'3c0=960(x2)=1920
	{0x25,0x38},	//VWin=h'438=1080
	{0x26,0x43},
	{0x27,0xFC},	//HwinSt=h'FC=252
	{0x28,0x16},	//VWinSt=h'16=22 
	{0x29,0x04},
	{0x2A,0xF0},
	{0x2B,0x14},
	{0x2C,0x02},
...
static const struct fps_info *jxf38p_get_fps_info(uint16_t fps)
{
	...
	for (i = 0; i < ARRAY_SIZE(g_jxf38p_fps_info_asic); i++)
		if (fps == (int)g_jxf38p_fps_info_asic[i].fps)
			break;
...
static uint16_t get_sensor_gain_reg(float fAGain, struct jxf38p_status *status)
{
	int wAgainReg;
...
static int jxf38p_set_mirror_flip(uint32_t isp_id,
...
static int jxf38_get_slave_addr_num(uint32_t isp_id)
{
...
static const struct rts_isp_sensor_ops jxf38p_ops = {
	...
	.set_mirror_flip = jxf38p_set_mirror_flip,
	.check = jxf38p_check,
	.get_slave_addr_num = jxf38_get_slave_addr_num,
};
