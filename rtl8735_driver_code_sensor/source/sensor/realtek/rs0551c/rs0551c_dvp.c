...
static struct rs0551c_dvp_status g_status[SUPPORTED_ISP_NUM] = {[0].hdr = LINEAR_MODE};
				...
				struct rts_isp_sensor_info *info)
{
	...
	info->crop.x = 0;
	info->crop.y = 0;
	info->crop.w = g_rs0551c_dvp_config.width;
	info->crop.h = g_rs0551c_dvp_config.height;
	...
	info->fps.fps[0] = g_rs0551c_dvp_config.fps;
	info->fps.num = 1;

	return RTS_ISP_OK;
}

static int rs0551c_dvp_get_init_info(uint32_t isp_id, float fps,
				     struct rts_isp_sensor_init_info *info)
{
	...
	debug("rs0551c dvp get fps %u init info\n", fps);
...
static int rs0551c_dvp_clk_set(void)
{
	...
	mipi_clk = (g_rs0551c_dvp_config.pclk << 1) / 1000000;
