...
static struct jxf53_status g_status[SUPPORTED_ISP_NUM] = {[0].hdr = LINEAR_MODE};

static const struct fps_info g_jxf53_fps_info_asic[] = {
	{30.0f, 2560, 86400000},
};

static struct rts_isp_i2c_reg g_jxf53_i2c_init_regs_asic[] = {
	{0x12,0x40},
	{0x48,0x8A},
	{0x48,0x0A},
	{0x0E,0x19},
...
static int jxf53_get_info(uint32_t isp_id, struct rts_isp_sensor_info *info)
{
	...
	info->modes.mode[0].fps = g_jxf53_fps_info_asic[0].fps;
	info->modes.num = 1;
...
#if (PROJECT_DOUBLEMINT)
	set_power_item(&up->items[i++], SNR_RST_GPIO, GPIO_LOW, 11000);
#else
...
{
	int i;
	for (i = 0; i < ARRAY_SIZE(g_jxf53_fps_info_asic); i++)
		if (fps == (int)g_jxf53_fps_info_asic[i].fps)
			break;
	if (i == ARRAY_SIZE(g_jxf53_fps_info_asic))
		return NULL;
		
	return &g_jxf53_fps_info_asic[i];
...
static uint16_t get_sensor_gain_reg(float fAGain, struct jxf53_status *status)
{
	int wAgainReg;
...
static int jxf53_set_mirror_flip(uint32_t isp_id,
...
static int jxf53_get_slave_addr_num(uint32_t isp_id)
{
...
static const struct rts_isp_sensor_ops jxf53_ops = {
	...
	.set_mirror_flip = jxf53_set_mirror_flip,
	.check = jxf53_check,
	.get_slave_addr_num = jxf53_get_slave_addr_num,
