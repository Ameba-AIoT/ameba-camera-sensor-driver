...
static struct jxf37_status g_status[SUPPORTED_ISP_NUM] = {[0].hdr = LINEAR_MODE};
...
static struct rts_isp_i2c_reg g_jxf37_i2c_init_regs_asic[] = {
	...
	{0x24,0xC4},	//HWin=h'3c4=964(x2)=1928
	{0x25,0x40},	//VWin=h'440=1088
	{0x26,0x43},	
	{0x27,0xC6},	//HwinSt=h'718=1816
	//{0x28,0x11},	//VWinSt=h'11=17
	{0x28,0x1B},	//VWinSt=h'11=17
	{0x29,0x04},
	{0x2A,0xBB},
	{0x2B,0x14},
	{0x2C,0x00},
...
static const struct fps_info *jxf37_get_fps_info(uint16_t fps)
{
	...
	for (i = 0; i < ARRAY_SIZE(g_jxf37_fps_info_asic); i++)
		if (fps == (int)g_jxf37_fps_info_asic[i].fps)
			break;
...
static uint16_t get_sensor_gain_reg(float fAGain, struct jxf37_status *status)
{
	int wAgainReg;
...
static int jxf37_set_mirror_flip(uint32_t isp_id,
...
static int jxf37_get_slave_addr_num(uint32_t isp_id)
{
...
static const struct rts_isp_sensor_ops jxf37_ops = {
	...
	.set_mirror_flip = jxf37_set_mirror_flip,
	.check = jxf37_check,
	.get_slave_addr_num = jxf37_get_slave_addr_num,
};
