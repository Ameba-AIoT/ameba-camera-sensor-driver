...
static struct gc05a2_status g_status[SUPPORTED_ISP_NUM] = {[0].hdr = LINEAR_MODE};

static const struct fps_info g_gc05a2_fps_info_asic[] = {
	{30.0f, 2940, 179200000},
};

static struct rts_isp_i2c_reg g_gc05a2_i2c_init_regs_asic[] = {
	{0x0315,0xd4},
	{0x0d06,0x01},
	{0x0a70,0x80},
	{0x031a,0x00},
...
static int gc05a2_get_info(uint32_t isp_id, struct rts_isp_sensor_info *info)
{
	...
	info->modes.mode[0].fps = g_gc05a2_fps_info_asic[0].fps;
...
static const struct fps_info *gc05a2_get_fps_info(uint16_t fps)
{
	int i;
		...
		for (i = 0; i < ARRAY_SIZE(g_gc05a2_fps_info_asic); i++)
			if (fps == g_gc05a2_fps_info_asic[i].fps)
				break;
		if (i == ARRAY_SIZE(g_gc05a2_fps_info_asic))
			return NULL;
		return &g_gc05a2_fps_info_asic[i];
				...
				struct rts_isp_sensor_init_info *info)
{
	...
	set_init_i2c_regs(info->sensor_regs[0], g_gc05a2_i2c_init_regs_asic, 0);
	...
	info->min_vts = status->min_vts = 2032;
	info->max_vts = 65536 - status->min_vts;
...
static uint16_t get_sensor_gain_reg(float fgain, struct gc05a2_status *status)
{
					 ...
					 struct rts_isp_sync_regs *regs)
{
	...
	//vts =  status->min_vts;
	vts = exp_gain->vts ;
	//vts = exp_gain->extra_dummy + status->min_vts;
	...
	if (abs(status->last_exposure - exp_gain->exposure[0]) > 0.001f){
		...
		/* exposure */
		exp_reg_value = clip_d_word(exposure_rows, 4, vts - 16);
...
static int gc05a2_set_mirror_flip(uint32_t isp_id,
								  struct rts_isp_sensor_mirror_flip *mf_info)
{
	...
	switch(bySnrImgDir)//
	{
		case 0: //normal
		default:
				set_sync_i2c(&reg[i++], 0x101, 0x0);			
		break;
		case 1: 	// Mirror
				set_sync_i2c(&reg[i++], 0x101, 0x1);				
		break;
		case 2: 		// VFlip
				set_sync_i2c(&reg[i++], 0x101, 0x2);
		break;
		case 3: 	// Rotate
				set_sync_i2c(&reg[i++], 0x101, 0x3);		
		break;
	}
	mf_info->regs.num = i;
	return RTS_ISP_OK;
...
static const struct rts_isp_sensor_ops gc05a2_ops = {
	...
	.set_mirror_flip = gc05a2_set_mirror_flip,
	.check = gc05a2_check,
