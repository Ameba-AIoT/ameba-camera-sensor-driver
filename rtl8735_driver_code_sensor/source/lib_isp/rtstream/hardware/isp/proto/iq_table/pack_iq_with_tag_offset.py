...
class IqGroup(object):
    def __init__(self, name, day_id, night_id, other_id):
        self.name = name
        print(day_id, night_id)
        self.day_id = day_id
...
def print_help():
    print('Usage: {} packed_iq.bin iq0_day.json iq0_night.json iq0_others.json ...'
...
iq_binaries = []
iq_bin_per_group = 3
for i in range(0, len(json_files), iq_bin_per_group):
    for dn_json in json_files[i:i + iq_bin_per_group]:
        if dn_json not in pb_index_dict:
            pb_index += 1
            pb_index_dict[dn_json] = pb_index
            iq_binaries.append(IqBinary(dn_json))
    iq_groups.append(IqGroup('IQ{}'.format(i // iq_bin_per_group),
                             pb_index_dict[json_files[i]],
                             pb_index_dict[json_files[i + 1]],
                             pb_index_dict[json_files[i + 2]]))
...
if iq_b_total + 4 > 200 * 1024:
    print("IQ tables exceed the size limit (200KB)", iq_b_total + 4)
header = struct.pack(header_fmt, 0x504B4951, 2, 0,
                     group_num, 0, binary_num, b'\0')
group = b''
for iq_g in iq_groups:
    group += struct.pack(group_fmt, bytes(iq_g.name, 'utf-8'),
                         iq_g.day_id, iq_g.night_id, iq_g.other_id, b'\0')
binary = b''
for iq_b in iq_binaries:
