...
def dump_bin(file_name: str, with_offset: bool=False):
    def json_name(index):
        return '{}_iq{}.json'.format(os.path.splitext(file_name)[0], index)
...
if __name__ == '__main__':
    if len(sys.argv) == 2:
        dump_bin(sys.argv[1], False)
    elif len(sys.argv) == 3:
        if sys.argv[2] == '0':
            dump_bin(sys.argv[1], False)
        elif sys.argv[2] == '1':
            dump_bin(sys.argv[1], True)
        else:
            print('Invalid with_offset: {}'.format(sys.argv[2]))
            sys.exit()
    else:
        print('Usage: {} iq_bin_file [with_offset]'.format(sys.argv[0])) # with_offset: [optional] 0 or 1. Default is 0.
        sys.exit()
