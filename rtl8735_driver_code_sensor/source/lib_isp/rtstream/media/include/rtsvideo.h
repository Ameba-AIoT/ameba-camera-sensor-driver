...
/**
 * @brief Create video process system channel
 * @return vps channel number, minus : fail
 */
...
enum RTS_ISP_CTRL_ID {
	...
	RTS_ISP_CTRL_ID_HUE, /**< hue (only rts3903/3913 support) */
	...
	RTS_ISP_CTRL_ID_WDR_MODE, /**< wide dynamic range mode */
	...
	RTS_ISP_CTRL_ID_FOV_MODE, /**< fov mode (only rts3903/3913 support) */
	...
	RTS_ISP_CTRL_ID_TM, /**< TM */
	RTS_ISP_CTRL_ID_RESERVED
};

/**
 * @brief ISP wide dynamic range mode
 */
enum RTS_ISP_WDR_MODE {
	RTS_ISP_WDR_DISABLE = 0, /**< wdr disable mode */
	RTS_ISP_WDR_MANUAL, /**< manual wdr mode */
	RTS_ISP_WDR_AUTO, /**< auto wdr mode */
...
struct rts_isp_af_ctrl {
	...
	const struct {
		uint32_t whole_win_sum; /**< whole window sum */
		uint32_t center_win_sum; /**< center window sum */
		uint32_t *win_sum; /**< window sum data */
...
/**
 * @brief Video process system control
 */
struct rts_vps_ctrl {
	uint32_t width; /**< VPS out width */
	uint32_t height; /**< VPS out height */
};

/**
 * @brief Set VPS control
 * @param[in] chnno VPS channel number
 * @param[in] ctrl pointer to rts_vps_ctrl
 * @return 0 : success, minus : fail
 */
int rts_av_set_vps_ctrl(unsigned int chnno, struct rts_vps_ctrl *ctrl);

/**
 * @brief Get VPS control
 * @param[in] chnno VPS channel number
 * @param[out] ctrl pointer to rts_vps_ctrl
 * @return 0 : success, minus : fail
 */
int rts_av_get_vps_ctrl(unsigned int chnno, struct rts_vps_ctrl *ctrl);
...
/**
 * @brief The RGB normalization and quantization control
 * @sa Tutorials: @ref hw_norm
 */
struct rts_rgb_norm_ctrl {
	/**
	...
	/**
	 * normaliztion scale with 8 bits left-shifting
	 * @sa @ref hw_norm
	 */
	uint16_t normal_scale;
	/**
	 * dynamic_fixed_point-8 quantization parameter.
	 * It can be caculated by 8 - fixed_point_pos.
	 * @sa @ref hw_norm
	 */
	int8_t quant_len;
	/**
	 * the inverse of 9 bits left-shifted asymmetric uint8 quantiztion scale
	 * @sa @ref hw_norm
	 */
	uint32_t asym_inv_scale;
	/**
	 * zero point of asymmetric uint8 quantization
	 * @sa @ref hw_norm
	 */
	uint8_t asym_zero_point;
};


/**
 * @brief Set RGB normalization control
 * @param[in] ctrl pointer to rts_rgb_norm_ctrl
 * @return 0 : success, minus : fail
 * @sa Tutorials: @ref hw_norm
 */
int rts_av_set_rgb_norm_ctrl(struct rts_rgb_norm_ctrl *ctrl);

/**
 * @brief Get RGB normalization control
 * @param[out] ctrl pointer to rts_rgb_norm_ctrl
 * @return 0 : success, minus : fail
 * @sa Tutorials: @ref hw_norm
 */
int rts_av_get_rgb_norm_ctrl(struct rts_rgb_norm_ctrl *ctrl);
...
/* sys video api -- vp */
/**
 * @brief Set the type of video process system
 * @param[in] val VP type, refer to \ref RTS_VP_TYPE
 * @return 0 : success, minus : fail
 */
int rts_av_sys_set_vp_type(int val);

/**
 * @brief Get the type of video process system
 * @return 0 : success, minus : fail
 */
int rts_av_sys_get_vp_type(void);
...
struct rts_h264_ctrl {
	...
	uint32_t color_space;  /**< 0 -> bt601, 1 -> bt709 */
 ...
 * @param[out] attr pointer to the address of rts_video_osd2_attr
 * @return 0 : success, minus : fail
 */
 ...
 * @param[in] attr pointer to rts_video_osd2_attr
 ...
 * @param[out] attr pointer to rts_video_osd2_attr
 * @param[in] blkidx index of OSD block
 * @return 0 : success, minus : fail
 */
int rts_av_get_osd2_single(struct rts_video_osd2_attr *attr, int buf_id, int blkidx);
 ...
 * @return 0 : success, minus : fail
 */
int rts_av_set_osd2(struct rts_video_osd2_attr *attr, int buf_id);
 ...
 * @return value of color table at specified index, minus : fail
 */
uint32_t rts_av_get_osd2_color_table(struct rts_video_osd2_attr *attr, int chn_id, int buf_id,
			int fmt, uint8_t r, uint8_t g, uint8_t b, uint8_t a);

/**
 * @brief release attribute
 * @param attr pointer to rts_video_osd2_attr
 */
void rts_av_release_osd2(struct rts_video_osd2_attr *attr);

int rts_av_set_osd2_special_config(struct rts_video_osd2_attr *attr,
			struct rts_osd2_scfg *scfg, int blkidx);
int rts_av_get_osd2_special_config(struct rts_video_osd2_attr *attr,
			struct rts_osd2_scfg *scfg, int blkidx);

void *rts_osd2_load_two_font_lib_info(void *pfontlib, void *lib_addr);
int rts_osd2_release_two_font_lib_info(void *pfontlib);
int rts_osd_find_char_bmp(void *ptr, struct rts_osd_char_t *pchar, BITMAP_S *result);
...
/**
 * @brief MJPEG ctrl parameter
 */
struct rts_mjpeg_ctrl {
	/**
	 * compress rate, 2 as default.
	 */
	uint16_t normal_compress_rate:8;

        /**
         * maximum compress rate, which is 15.
         */
	const uint16_t ncr_max;
        /**
         * minimum compress rate, which is 2.
         */
	const uint16_t ncr_min;
	const uint16_t ncr_step; /**< compress rate step, which is 1 */

	uint32_t reserved[4];
};

/**
 * @brief Initialize and obtain the MJPEG control
 * @param[in] chnno MJPEG channel number
 * @param[out] ppctrl pointer to the address of rts_mjpeg_ctrl
 * @return 0 : success, minus : fail
 */
...
/**
 * @brief Release the MJPEG control
 * @param[in] ctrl pointer to rts_mjpeg_ctrl
 * @attention After using rts_av_query_mjpeg_ctrl, rts_av_release_mjpeg_ctrl
 * must be called to release the resources.
 */
void rts_av_release_mjpeg_ctrl(struct rts_mjpeg_ctrl *ctrl);

/**
 * @brief Get the MJPEG control
 * @param[out] ctrl pointer to rts_mjpeg_ctrl
 * @return 0 : success, minus : fail
 */
int rts_av_get_mjpeg_ctrl(struct rts_mjpeg_ctrl *ctrl);

/**
 * @brief Set the MJPEG control
 * @param[in] ctrl pointer to rts_mjpeg_ctrl
 * @return 0 : success, minus : fail
 */
...
/**
 * @brief MJPEG decoder ctrl parameter
 */
struct rts_mjpeg_dec_ctrl {
	uint8_t h_scale; /**< horizontal scale, valid value:0-3 */
	uint8_t v_scale; /**< vertical scale, valid value:0-3 */
};

/**
 * @brief Initialize and obtain the MJPEG decoder control
 * @param[in] chnno MJPEG decoder channel number
 * @param[out] ppctrl pointer to the address of rts_mjpeg_dec_ctrl
 * @return 0 : success, minus : fail
 */
int rts_av_query_mjpeg_dec_ctrl(unsigned int chnno,
			struct rts_mjpeg_dec_ctrl **ppctrl);

/**
 * @brief Release the MJPEG decoder control
 * @param[in] ctrl pointer to rts_mjpeg_dec_ctrl
 * @attention After using rts_av_query_mjpeg_dec_ctrl,
 * rts_av_release_mjpeg_dec_ctrl must be called to release the resources.
 */
void rts_av_release_mjpeg_dec_ctrl(struct rts_mjpeg_dec_ctrl *ctrl);

/**
 * @brief Get the MJPEG decoder control
 * @param[out] ctrl pointer to rts_mjpeg_dec_ctrl
 * @return 0 : success, minus : fail
 */
int rts_av_get_mjpeg_dec_ctrl(struct rts_mjpeg_dec_ctrl *ctrl);

/**
 * @brief Set the MJPEG decoder control
 * @param[in] ctrl pointer to rts_mjpeg_dec_ctrl
 * @return 0 : success, minus : fail
 */
int rts_av_set_mjpeg_dec_ctrl(struct rts_mjpeg_dec_ctrl *ctrl);

/**
 * @brief MJPEG decoder ctrl parameter for simple API
 */
struct rts_jpgdec_ctrl {
	/**
	 * output yuv format,supported value is as follows:\n
	 * RTS_AV_FMT_UNDEFINED: not specified,determined by original yuv format;\n
	 * RTS_V_FMT_YUYV:yuyv;\n
	 * RTS_V_FMT_YVYU:yvyu;\n
	 * RTS_V_FMT_YUV420PLANAR:420-planar;\n
	 * RTS_V_FMT_YUV420SEMIPLANAR:nv12,default value;\n
	 * RTS_V_FMT_YUV422SEMIPLANAR:nv16
	 */
	uint32_t out_fmt;
	uint8_t h_scale; /**< horizontal scale, valid value:0-3 */
	uint8_t v_scale; /**< vertical scale, valid value:0-3 */

	uint8_t roi; /**< enable ROI */
	uint32_t roi_x; /**< X coordinates for ROI */
	uint32_t roi_y; /**< Y coordinates for ROI */
	uint32_t roi_w; /**< width for ROI */
	uint32_t roi_h; /**< height for ROI */
};

/**
 * @brief MJPEG decoder simple API to decode one MJPEG picture
 * @param[in] inbuf input data
 * @param[out] outbuf output data
 * @param[in] pctrl pointer to rts_jpgdec_ctrl
 * @attention Inbuf length must be a multiple of 1024. The maximum size is 16383 x 1024 bytes.
 * @return 0 : success, minus : fail
 */
int rts_av_jpg_decode(struct rts_av_buffer *inbuf,
			struct rts_av_buffer *outbuf,
			struct rts_jpgdec_ctrl *pctrl);

/**
 * @brief MJPEG decoder simple API to get the size of output data
 * @param[in] inbuf input data
 * @param[in] pctrl pointer to rts_jpgdec_ctrl
 * @attention Inbuf length must be a multiple of 1024. The maximum size is 16383 x 1024 bytes.
 * @return size of output data, minus : fail
 */
int rts_av_jpg_get_dec_outlen(struct rts_av_buffer *inbuf,
			struct rts_jpgdec_ctrl *pctrl);
/*!@}*/
