...
enum RTS_AV_ROTATION {
	...
	RTS_AV_ROTATION_180 = 3, /**< rotation 180 degree */
};

/**
 * @brief mirror description
 */
enum RTS_AV_MIRROR {
	RTS_AV_MIRROR_NO = 0, /**< no mirror */
	RTS_AV_MIRROR_VER, /**< mirror vertical */
	RTS_AV_MIRROR_HOR, /**< mirror horizontal */
	RTS_AV_MIRROR_HOR_VER, /**< mirror horizontal and vertical */
};
...
struct rts_av_buffer {
	...
	uint32_t type; /**< audio encode type or video frame type */
...
/**
 * @brief buffer attribute
 */
struct rts_av_buffer_attr {
	enum RTS_AV_BUFFER_NAME name; /**< buffer name */
	uint8_t cacheable; /**< buffer cacheable */
};
...
typedef struct rts_av_profile rt_av_profile_st;
...
#define RTS_AV_ID_VPS			rts_av_fourcc('v', 'p', 's', '0')
 ...
 */
struct rts_vin_attr {
	int vin_buf_num; /**< video-in outbuf number */
	int vin_id; /**< Video-in ID, it will be used in create vin channel */
};
/*!@}*/
	...
	struct {
		...
		struct {
			...
			struct {
				...
				bool cache; /**< cacheable or uncacheable */
				/**
...
/**
 * @brief Type for video process unit
 */
enum RTS_VP_TYPE {
	RTS_SYS_VP_TYPE_ISP = 0, /**< for ISP */
	RTS_SYS_VP_TYPE_VPS /**< for video process system */
};
/*!@}*/
#ifdef __cplusplus
}
