...
void rtscam_get_timestamp(uint64_t *ts)
{
	...
	*ts = (uint64_t)1000000 * xTaskGetTickCount() / portTICK_PERIOD_MS;
	*ts *= (uint64_t)1000000;
}
			   ...
			   struct rtscam_video_format_xlate *pfmt)
{
	...
	if (find_format_by_fourcc(stream, pfmt->fourcc)) {
		printf("%c%c%c%c has been registered\n", v4l2pixfmtstr(pfmt->fourcc));
		return -EEXIST;
	}

	format = rts_calloc(1, sizeof(*format));
	if (NULL == format)
		return -ENOMEM;
				   ...
				   __u32 fourcc, struct rtscam_frame_size *size)
{
	...
	frame = rts_calloc(1, sizeof(*frame));
	if (NULL == frame)
		return -ENOMEM;
				   ...
				   struct rtscam_frame_size *step)
{
	...
	if (format->initialized &&
		...
		printf("initialized frame_type error %d \n", format->frame_type );
		return -EINVAL;
	}
