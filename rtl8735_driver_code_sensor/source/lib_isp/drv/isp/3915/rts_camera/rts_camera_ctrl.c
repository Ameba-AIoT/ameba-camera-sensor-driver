...
{
#if 0
	__u8 bits = mapping->size;
	__u8 offset = mapping->offset;
	__s32 value = 0;
	__u8 mask;
	...
	for (; bits > 0; data++) {
		__u8 byte = *data & mask;
		value |= offset > 0 ? (byte >> offset) : (byte);
			 ...
			 struct rtscam_video_ctrl_info *info)
{
	...
	ctrl->ctrl_data = rts_calloc(1, ctrl->info.size * RTS_CTRL_DATA_LAST + 1);
		...
		struct rtscam_video_ctrl_mapping **mapping)
{
	...
	*mapping = NULL;
	...
	list_for_each_entry(ctrl, &icd->controls, list, struct rtscam_video_ctrl) {
		if (!ctrl->initialized)
			continue;

		list_for_each_entry(map, &ctrl->info.mappings, list, struct rtscam_video_ctrl_mapping) {
			     ...
			     struct v4l2_queryctrl *v4l2_ctrl)
{
	...
	case V4L2_CTRL_TYPE_MENU:
		...
		for (i = 0; i < mapping->menu_count; ++i, ++menu) {
			if (menu->value == (u32)v4l2_ctrl->default_value) {
...
//		return -ERESTARTSYS;
//	}
	ret = xSemaphoreTake(icd->ctrl_lock, ISP_SEM_TIMEOUT);
	if (ret != pdTRUE) {
		isp_error("acquire ctrl lock fail\n");
		return -ERESTARTSYS;
	}
...
//	mutex_unlock(&icd->ctrl_lock);
	//osMutexRelease(icd->ctrl_lock);
	xSemaphoreGive(icd->ctrl_lock);
...
//	return mutex_lock_interruptible(&icd->ctrl_lock) ? -ERESTARTSYS : 0;
	return (xSemaphoreTake(icd->ctrl_lock, ISP_SEM_TIMEOUT) != pdTRUE) ? -ERESTARTSYS : 0;
}
			   ...
			   struct v4l2_ext_control *xctrl)
{
	...
	case V4L2_CTRL_TYPE_BOOLEAN:
		...
		break;
	case V4L2_CTRL_TYPE_MENU:
		if (xctrl->value < 0 || xctrl->value >= (s32)mapping->menu_count)
			return -ERANGE;
...
int rtscam_video_release_ctrl(struct rtscam_video_device *icd)
{
	...
	list_for_each_entry_safe(ctrl, tmp, &icd->controls, list, struct rtscam_video_ctrl) {
		list_del(&ctrl->list);
