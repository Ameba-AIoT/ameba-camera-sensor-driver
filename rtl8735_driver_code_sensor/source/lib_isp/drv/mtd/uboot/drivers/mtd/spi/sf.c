...
int spi_flash_set_prm_mode(struct spi_slave *spi)
{
	return spi_flash_set_prm(spi);
}
...
int spi_flash_set_reset_test(struct spi_slave *spi, u8 channel, u8 cmd)
{
	return flash_set_reset_test(spi, channel, cmd);
}

int spi_flash_flush_rst_fifo(struct spi_slave *spi)
{
	return flash_flush_rst_fifo(spi);
}

int spi_flash_set_rst_fifo(struct spi_slave *spi, u16 reset_flow)
{
	return flash_set_rst_fifo(spi, reset_flow);
}

int spi_flash_enable_prm(struct spi_slave *spi)
{
	return flash_enable_prm(spi);
}

int spi_flash_disable_prm(struct spi_slave *spi)
{
	return flash_disable_prm(spi);
}

int spi_flash_exit_prm_mode(struct spi_slave *spi)
{
	return flash_exit_prm(spi);
}
