...
int spi_flash_cmd_write_status(struct spi_flash *flash, u8 sr)
{
	...
	if (ret < 0) {
		rt_debug("SF: fail to write status register\n");
		return ret;
	}
...
int spi_flash_cmd_read_status(struct spi_flash *flash, u8 *sr, u8 cmd)
{
	...
	if (ret < 0) {
		rt_err("SF: fail to read status register\n");
		return ret;
	}
...
static int spi_flash_cmd_bankaddr_write(struct spi_flash *flash, u8 bank_sel)
{
	...
	if (flash->bank_curr == bank_sel) {
		rt_err("SF: not require to enable bank%d\n", bank_sel);
		return 0;
	}
	...
	if (ret < 0) {
		rt_err("SF: fail to write bank register\n");
		return ret;
	}
...
/*	unsigned long timebase;*/
	...
	if (ret) {
		rt_err("SF: fail to read %s status register\n", cmd == CMD_READ_STATUS_1 ? "read" : "flag");
		return ret;
	}
...
*/
	...
	if (ret) {
		rt_err("SF: unable to claim SPI bus\n");
		return ret;
	}

	ret = spi_flash_cmd_write_enable(flash);
	if (ret < 0) {
		rt_err("SF: enabling write failed\n");
		return ret;
	}

	ret = spi_flash_cmd_write(spi, cmd, cmd_len, buf, buf_len);
	if (ret < 0) {
		rt_err("SF: write cmd failed\n");
		return ret;
	}
...
int spi_flash_cmd_erase_ops(struct spi_flash *flash, u32 offset, size_t len)
{
	...
	if (offset % erase_size || len % erase_size) {
		rt_err("SF: Erase offset/length not multiple of erase size\n");
		return -1;
	}
		...
		size_t len, const void *buf)
{
	...
	flash->spi->max_write_size = 0x100;
#else
	...
	for (actual = 0; actual < len; actual += chunk_len) {
		...
		// my_debug("PP: 0x%p => cmd = { 0x%02x 0x%02x%02x%02x } chunk_len = %zu\n",
...
/*	rt_printf("read common %x, %x, %x, %x, %x\n", cmd[0], cmd[1], cmd[2], cmd[3], cmd[4]);*/
	ret = spi_claim_bus(flash->spi);
	if (ret) {
		rt_err("SF: unable to claim SPI bus\n");
		return ret;
...
#endif
		/*spic FIFO size is 128 byte*/
		read_len = MIN(0x80, len);
...
int spi_flash_exit_prm_ops(struct spi_flash *flash)
{
	int ret = 0;

	ret = spi_flash_exit_prm_mode(flash->spi);
	spi_flash_disable_prm(flash->spi);
...
static int sst_byte_write(struct spi_flash *flash, u32 offset, const void *buf)
{
	...
	//my_debug("BP[%02x]: 0x%p => cmd = { 0x%02x 0x%06x }\n",
...
int sst_write_wp(struct spi_flash *flash, u32 offset, size_t len, const void *buf)
{
	...
	for (; actual < len - 1; actual += 2) {
		//my_debug("WP[%02x]: 0x%p => cmd = { 0x%02x 0x%06x }\n",
		...
		if (ret) {
			rt_err("SF: sst word program failed\n");
			break;
		}
	...
	//my_debug("SF: sst: program %s %zu bytes @ 0x%zx\n",	ret ? "failure" : "success", len, offset - actual);
