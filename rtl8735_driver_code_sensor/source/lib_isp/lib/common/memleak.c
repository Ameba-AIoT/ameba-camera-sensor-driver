...
static unsigned int history_length = 0;
...
struct head
{
	...
	union {
		...
		struct { char *file; unsigned long line; const char *func; } free;
	} in;
};
...
int dbg_check_addr(char *msg, void *addr, int opt)
{
	...
	if (opt & CHK_ALLOC)
		...
		{
			fprintf(stderr, "%s:%lu: %s: allocated %p (alloc: %s:%lu size: %lu)\n",
					dbg_file_name, dbg_line_number, msg, addr, p->file, p->line, (unsigned long)p->size);
			return CHK_ALLOC;
		}
	if (opt & CHK_FREED)
		if ((p = find_in_hist(addr)))
		{
			fprintf(stderr, "%s:%lu: %s: freed %p (alloc: %s:%lu size: %lu free: %s:%lu)\n",
					dbg_file_name, dbg_line_number, msg, addr, p->file, p->line,
					(unsigned long)p->size, p->in.free.file, p->in.free.line);
			return CHK_FREED;
		}
	if (opt) fprintf(stderr, "%s:%lu: %s: unknown %p\n", dbg_file_name, dbg_line_number, msg, addr);
	return 0;
}
...
void dbg_heap_dump(char *key)
{
	...
	printf("Allocated & Not Freed: (Using or Leak...)\n");
	printf("--------------------+-------------------------------------------------\n");
	printf("%10s %8s | %20s %5s %20s\n", "addr", "size", "(alloc) file", "line", "function");
	printf("--------------------+-------------------------------------------------\n");

	p = first;
	while (p)
	{
		sprintf(buf, "%p %8u | %20s %5lu %20s\n", p->addr, p->size, p->file, p->line, p->func);
		p = p->in.list.next;
...
void dbg_history_dump(char *key)
{
	...
	if (history_length)
	{
		printf("Allocated & Freed: (Finish)\n");
		printf("--------------------+----------------------------+----------------------------\n");
		printf("%10s %8s | %20s %5s | %20s %5s\n", "addr", "size", "(alloc) file", "line", "(free) file", "line");
		printf("--------------------+----------------------------+----------------------------\n");
		...
		while (cnt--)
		{
			sprintf(buf, "%p %8u | %20s %5lu | %20s %5lu\n",
					p->addr, p->size, p->file, p->line, p->in.free.file, p->in.free.line);
			if (strstr(buf, key)) fputs(buf, stderr);
			ADVANCE(p);
		}
		printf("\n");
